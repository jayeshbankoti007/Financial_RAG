...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        clustering_algorithm = st.selectbox(
            ...
            index=0,
            help="""
            • HDBSCAN: Best quality, automatically finds optimal clusters
            • DBSCAN: Good quality, faster than HDBSCAN
            """,
        )
        ...
        if api_key:
            ...
            st.warning("⚠️ No API Key - Will use text extraction fallback")
...
def render_source_information(retrieved_chunks: List[Tuple]):
    ...
    for i, (chunk, score) in enumerate(retrieved_chunks[:7]):
        with st.expander(
            f"📄Page {chunk.page_numbers} (Relevance: {score:.3f})", expanded=(i == 0)
        ):
            st.write(chunk.text)
...
def render_welcome_screen():
    ...
    st.markdown("---")
    st.info(
        "💡 **Pro Tip**: For best results, use financial accounting textbooks, annual reports, or similar structured documents."
    )
...
def main():
    ...
    st.set_page_config(
        page_title="Financial RAG Q&A System", page_icon="📊", layout="wide"
    )
